...
import '../services/team_message_service.dart'; // ✅ استخدم نفس الملف الموجود عندك

class NewClubMessageScreen extends StatefulWidget {
  const NewClubMessageScreen({super.key});

  @override
  State<NewClubMessageScreen> createState() => _NewClubMessageScreenState();
}

class _NewClubMessageScreenState extends State<NewClubMessageScreen> {
  ...
  final List<Map<String, dynamic>> targets = [
    {'id': '1', 'label': 'رئيس النادي'},
    {'id': '2', 'label': 'أمين سر النادي'},
  ];
  ...
  Future<void> submitMessage() async {
    if (selectedTarget == null || titleController.text.isEmpty || messageController.text.isEmpty) {
    ...
    final response = await TeamMessageService.sendMessageToClub(
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('إرسال رسالة إلى النادي', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          centerTitle: true,
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: targets.map<DropdownMenuItem<String>>((role) {
